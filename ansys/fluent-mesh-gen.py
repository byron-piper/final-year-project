...
from sample.helper import load_parameters, calc_first_cell_height
...
target_yplus = parameters["solver"]["target_yplus"]
...
converted_geometries_folder = os.path.join(geometries_folder, "meshed")

overwrite = parameters["mesh"]["overwrite"]

#endregion

#region # ==== CALCULATE BOUNDARY LAYER PROPERTIES ==== #

bl_first_height = calc_first_cell_height(freestream_reynolds, chord, freestream_density, freestream_viscosity, target_yplus)
...
if not os.path.exists(converted_geometries_folder):
	os.mkdir(converted_geometries_folder)
...
for i, geometry in enumerate(geometries):
	...
	try:
		...
		logging.error("{0}: {1} | Problem when during meshing process. Skipping!".format(progress, mesh_name))