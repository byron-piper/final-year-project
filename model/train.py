...
from datasets import FluentDataset, FlowFieldDataset
from models import VAE, Autoencoder, ConvAutoencoder, ConvAutoencoderLinear
...
def train_model(params:dict, logger:logging.Logger, model_dict:dict, model_instance, save_model:bool=False):
    ...
    for epoch in range(num_epochs):
        model.train()
        for batch_idx, (field, _, _) in enumerate(train_loader):
            batch_item_start_time = time.time()
            field = field.view(batch_size, 1, 320, 320)
        ...
        logger.info(f"Time Elapsed: {(time.time() - start_time)/60:.4f} min")
...
if __name__ == "__main__":
    ...
    model_dict = train_vae(params, logger, model_dict, save_model=True)